...
def send_download_links(*, receiver_email: str, message: list):
    ...
    try:
        ...
        print(message)
        link_for_desktop_cassa_windows = u'<a href="{mes}">CLICK TO DOWNLOAD WINDOWS DESKTOP CASSA</a>'.format(mes=message[0][0]) \
        if message[0] else ''
        link_for_desktop_cassa_linux = u'<a href="{mes}">CLICK TO DOWNLOAD LINUX DESKTOP CASSA</a>'.format(mes=message[0][0]) \
        \
        if message[0] else ''
